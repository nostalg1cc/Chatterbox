...
export function MessageTemplate({ name, avatar, avatarDecoration, nameDecoration, nameColor, nameFont, nameWeight, message, timestamp, isSelf, showMeta = true, media, sourceMessage, isDeleted = false, isEdited = false, decorationActive = false, onDecorationHoverChange }) {
  const className = ["message-template", isSelf && "message-template--self", !showMeta && "message-template--grouped"].filter(Boolean).join(" ");
  ...
  return (
    <article className={className} aria-label={"Message from " + name + " at " + timestamp} onPointerEnter={() => { setHovered(true); onDecorationHoverChange?.(true); }} onPointerLeave={() => { setHovered(false); onDecorationHoverChange?.(false); }}>
