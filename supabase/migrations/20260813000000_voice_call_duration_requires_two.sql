-- "Call lasted" should only count time when both participants were actually
-- in the room together, not solo time spent waiting for the other person to
-- join. Track the moment the room first reaches 2 participants, and base the
-- duration on that instead of on when the room was created.
...
begin
  ...
  if found and v_existing.conversation_id = p_conversation_id and v_existing.session_id = p_session_id then
    ...
    -- The room just reached 2 participants for the first time this call -
    -- mark it so the eventual duration only counts from here.
...
begin
  ...
  select both_joined_at into v_both_joined_at from public.voice_rooms where conversation_id = v_conversation_id;
  if not exists (select 1 from public.voice_participants p where p.conversation_id = v_conversation_id) then
    ...
    v_room_ended := true;
    -- Only count time the two of you were actually in the room together -
    -- if the other person never joined, there's nothing to count.
    v_duration_seconds := case
      when v_both_joined_at is null then 0
      else greatest(0, floor(extract(epoch from (now() - v_both_joined_at)))::integer)
    end;
