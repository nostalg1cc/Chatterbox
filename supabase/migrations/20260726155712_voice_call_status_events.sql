-- Durable, RLS-safe status entries for the lifecycle of a voice room.
-- The database emits them atomically: exactly one start per room generation and
-- one end only when the final participant deliberately leaves.

alter table public.messages
  add column if not exists message_kind text not null default 'chat'
    check (message_kind in ('chat', 'voice_started', 'voice_ended')),
  add column if not exists voice_duration_seconds integer
    check (voice_duration_seconds is null or voice_duration_seconds >= 0);

-- System rows are written only by the guarded voice-room functions below.
...
create policy "messages_insert" on public.messages
  ...
  with check (
    ...
    and voice_duration_seconds is null
  );

drop policy if exists "messages_update_own" on public.messages;
create policy "messages_update_own" on public.messages
  for update to authenticated
  using (sender_id = (select auth.uid()) and message_kind = 'chat')
  with check (sender_id = (select auth.uid()) and message_kind = 'chat');
...
begin
  ...
  if found and v_existing.conversation_id = p_conversation_id and v_existing.session_id = p_session_id then
    ...
    update public.voice_rooms set updated_at = now() where conversation_id = p_conversation_id;
  end if;
...
begin
  ...
  select started_at into v_started_at from public.voice_rooms where conversation_id = v_conversation_id;
  if not exists (select 1 from public.voice_participants p where p.conversation_id = v_conversation_id) then
    delete from public.voice_rooms where conversation_id = v_conversation_id;
    v_room_ended := true;
    v_duration_seconds := greatest(0, floor(extract(epoch from (now() - v_started_at)))::integer);
    insert into public.messages (conversation_id, sender_id, content, message_kind, voice_duration_seconds)
    values (v_conversation_id, v_user_id, 'Voice chat ended', 'voice_ended', v_duration_seconds);
  else
    update public.voice_rooms set updated_at = now() where conversation_id = v_conversation_id;
  end if;

  return jsonb_build_object('status', 'left', 'conversation_id', v_conversation_id, 'room_ended', v_room_ended, 'duration_seconds', v_duration_seconds);
end;
$$;