...
type RequestBody = {
  mode?: "list" | "reserve" | "finalize" | "discard" | "delete" | "play";
  soundId?: string;
  name?: string;
  sizeBytes?: number;
  durationMs?: number;
...
async function activeVoiceMembers(
  admin: any,
  conversationId: string,
  userId: string
): Promise<string[] | null> {
...
const handler = withSupabase({ auth: "user" }, async (req, ctx) => {
  ...
  if (body.mode === "reserve") {
    ...
    if (!name || !Number.isFinite(size) || size < 1 || size > MAX_BYTES ||
        !Number.isFinite(duration) || duration < 100 || duration > MAX_DURATION_MS) {
    ...
    const { data: reserved, error: reserveError } = await admin.rpc(
      "reserve_soundboard_upload",
      { p_sound_id: body.soundId, p_owner_id: userId, p_path: path, p_size_bytes: size }
    );
    if (reserveError) return json({ error: reserveError.message }, 500);
    if (!reserved) {
      return json({ error: "Soundboard quota reached (24 clips, 16 MiB/user, 96 MiB total)." }, 409);
    }
  ...
  if (body.mode === "play") {
    ...
    const { data: sound, error: soundError } = await admin
      .from("soundboard_sounds")
      .select("*")
      .eq("id", body.soundId)
      .in("owner_id", memberIds)
    ...
    const { data, error } = await admin.storage
      .from(BUCKET)
      .createSignedUrl(sound.storage_path, 60);
    if (error || !data?.signedUrl) {
      return json({ error: error?.message ?? "Sound could not be opened" }, 500);
    }
    return json({
      sound: {
        id: sound.id,
  ...
  if (body.mode === "finalize") {
    ...
    const size = Math.floor(Number(body.sizeBytes));
    const duration = Math.floor(Number(body.durationMs));
    if (!name || size < 1 || size > MAX_BYTES || duration < 100 || duration > MAX_DURATION_MS) {
      return json({ error: "Invalid sound metadata" }, 400);
    }
    ...
    const [{ data: objects, error: listError }, { data: reservation }] = await Promise.all([
      ...
      admin.rpc("soundboard_reservation_bytes", {
        p_sound_id: body.soundId,
        p_owner_id: userId,
      }),
    ]);
    ...
    const { data, error } = await admin.from("soundboard_sounds").upsert({
      id: body.soundId,
      owner_id: userId,
      name,
      storage_path: path,
